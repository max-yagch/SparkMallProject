package yang.sparkmall.mock.util.realtime

import java.util.Properties

import org.apache.kafka.clients.producer.{KafkaProducer, ProducerConfig, ProducerRecord}
import yang.sparkmall.common.model.CityInfo
import yang.sparkmall.common.util.ConfigUtil
import yang.sparkmall.mock.util.{RanOpt, RandomOptions}
...
object MockRealTimeGenerator {
	...
	def main(args: Array[String]): Unit = {
		...
		val broker =ConfigUtil.getValueByKey("kafka.broker.list")
		//println(broker)
		val topic =  "ads_log"
		// 创建Kafka消费者
